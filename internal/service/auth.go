...
type AuthService interface {
	SignIn(loginDTO dto.LoginDTO) (*datastruct.UserModel, *string, error)
	SignUp(model datastruct.UserModel) (*datastruct.UserModel, *string, error)
...
func (a *authService) SignIn(loginDTO dto.LoginDTO) (*datastruct.UserModel, *string, error) {
	...
	if err != nil {
		...
		return userData, &jwt.Token, nil
	}
}

func (a *authService) SignUp(model datastruct.UserModel) (*datastruct.UserModel, *string, error) {

	if !utils.IsEmailValid(model.Email) {
		return nil, nil, errors.New("email is not valid")
	}
